...
- **Godot resource parsing**: tree-sitter-godot-resource v0.7.0 (parses .tscn, .tres, project.godot)
- **AST type generation**: @asgerf/dts-tree-sitter v0.21.0 (generates typed AST interfaces from tree-sitter grammars)
...
- typescript ^5.9.0, tree-sitter ^0.22.0, tree-sitter-gdscript ^6.1.0, tree-sitter-godot-resource ^0.7.0
- source-map ^0.7.4, commander ^13.0.0, chokidar ^4.0.0
- Dev: @types/node ^22.0.0, vitest ^3.0.0, @asgerf/dts-tree-sitter ^0.21.0
...
scripts/
  fix-tree-sitter-types.js  # Post-processes @asgerf/dts-tree-sitter output for TS 5.9 compatibility
...
src/
  ...
  parser/
    gdscript/          # tree-sitter wrapper for GDScript
      index.ts         # GDScriptParser class
      types.ts         # AUTO-GENERATED by @asgerf/dts-tree-sitter: SyntaxType enum, typed node interfaces with field accessors
    godot-resource/    # tree-sitter wrapper for .tscn/.tres/project.godot
      index.ts         # GodotResourceParser class
      types.ts         # AUTO-GENERATED by @asgerf/dts-tree-sitter: resource file AST types
...
- [x] TS-to-GD converter with 21 fixture tests passing
...
- [x] AST types generated by @asgerf/dts-tree-sitter from tree-sitter grammars (gdscript + godot-resource)
  - `SyntaxType` const object (not enum, for `erasableSyntaxOnly` compat), typed node interfaces with field accessors
  - Post-processed by `scripts/fix-tree-sitter-types.js`: `export class` → `export declare class`, `const enum` → const object + type alias, `SyntaxType.X` → `typeof SyntaxType.X` in type positions
  - Regenerate: `yarn generate:ast-types` (gdscript), `yarn generate:resource-ast-types` (godot-resource)
- [x] GD-to-TS converter with typed AST and 22 fixture tests:
...
- [x] Typings generation (classes, scenes, godot-docs with override system)
  - Scene parsing via tree-sitter-godot-resource (replaces regex): .tscn, .tres, project.godot
  - `get_node()`/`get_node_or_null()` overloads via module augmentation per script class
  - `get_parent()` typed via `[__parent]` symbol on `Node<Tree>` generic: Godot built-in types use `<{[__parent]: ParentType}>`, user script classes use module augmentation
  - `GodotResources` interface: `.tscn` → `PackedScene<T>`, `.gd` → `typeof _Class` (class/constructor type, not instance), `.tres` → parsed `gd_resource type` header for precise typing, other assets → inferred from extension
  - Autoload singletons from `project.godot` `[autoload]` section → `const GlobalName: Type`
...
#### GD node paths and scene typings

Parses `.tscn` files (via tree-sitter-godot-resource) to generate typed overloads:
- `get_node("Path")` / `get_node_or_null("Path")` — returns the correct Godot type for known scene paths
- `get_parent()` — returns typed parent via `[__parent]` symbol on `Node<Tree>` generic
  - User script classes: module augmentation adds `get_parent(): ParentScriptType`
  - Godot built-in types: `Sprite2D<{[__parent]: Player}>` encodes parent in the generic
- `GodotResources` interface maps `res://` paths to loaded types:
  - `.tscn` → `PackedScene<RootScriptType>` (typed instantiation)
  - `.gd` → `typeof ScriptClass` (class/constructor type for `load()`/`preload()`)
  - `.tres` → parsed from `[gd_resource type="..."]` header (e.g. `ShaderMaterial`)
  - Other assets (`.png`, `.ogg`, etc.) → inferred resource type from extension
- Autoload singletons from `project.godot` `[autoload]` section → global `const` declarations
...
#### TS-to-GD test fixtures (21 paired .ts/.gd files in tests/fixtures/ts-to-gd/)

abstract, annotations, anonym, arrays-dicts, await-coroutines, classes, comments, const, constructor, control-flow, dict, enums, functions, lambda, match, operators, signals, static-members, strings, type-casting, variables

#### GD-to-TS test fixtures (22 paired .gd/.ts files in tests/fixtures/gd-to-ts/)

abstract, annotations, anonym, comments, const, constructor, control-flow, dict, enums, expressions, functions, inherit-user, inherit-user-base, lambda, match, operators, operators2, self, self2, signal-handlers, signals, variables
...
- `scene-typings.test.ts` — Scene typings generation: globals.d.ts, get_node/get_parent overloads, [__parent] types, GodotResources (PackedScene, typeof class, .tres types), autoloads, tsc compilation
...
- Dict string literal keys: use `emitStringLiteral()` for proper quote escaping (e.g. `'it\'s'` → `"it's"`, `"say \"hello\""` preserved)
- `[__parent]` type encoding: `Node<Tree extends object = any>` with `get_parent()` using conditional type `[any] extends [{[__parent]: infer P}] ? P : Node` — `any` default infers `unknown` → returns `Node`; specific `{[__parent]: Player}` infers `Player`
- GodotResources `.gd` entries: use `typeof _Class` (class/constructor type) not `_Class` (instance type) — `load("res://Player.gd")` returns the script/class, not an instance
- `@asgerf/dts-tree-sitter` post-processing: `export class` → `export declare class` (ambient), `const enum` → const object (TS 5.9 `erasableSyntaxOnly`), `SyntaxType.X` in type positions → `typeof SyntaxType.X`
- tree-sitter-gdscript `ParametersNode` name clash: generated types have both a type alias (union of parameter children) and an interface (the `parameters` node); type alias renamed to `ParameterChildNode`

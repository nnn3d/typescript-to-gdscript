...
declare class NavigationAgent2D<Tree extends object = any> extends Node<Tree> {
  ...
  /**
   * If {@link debug_use_custom} is `true` uses this line width for rendering paths for this agent instead of global line width.
   */
   ...
   * Does not affect normal pathfinding. To change an actor's pathfinding radius bake {@link NavigationPolygon} resources with a different {@link NavigationPolygon.agent_radius} property and use different navigation maps for each actor size.
   */
  radius: float;
  ...
  /** Returns the {@link RID} of this agent on the {@link NavigationServer2D}. */
  get_rid(): RID;
  /**
   ...
   * - `type`: Always {@link NavigationPathQueryResult2D.PATH_SEGMENT_TYPE_LINK}.
   * - `rid`: The {@link RID} of the link.
   * - `owner`: The object which manages the link (usually {@link NavigationLink2D}).
   * - `link_entry_position`: If `owner` is available and the owner is a {@link NavigationLink2D}, it will contain the global position of the link's point the agent is entering.
   * - `link_exit_position`: If `owner` is available and the owner is a {@link NavigationLink2D}, it will contain the global position of the link's point which the agent is exiting.
   */
   ...
   */
  velocity_computed: Signal<[Vector2]>;
  /**

...
declare class NavigationPathQueryParameters3D extends RefCounted {
  /**
   * The list of region {@link RID}s that will be excluded from the path query. Use {@link NavigationRegion3D.get_rid} to get the {@link RID} associated with a {@link NavigationRegion3D} node.
  ...
  /**
   * The list of region {@link RID}s that will be included by the path query. Use {@link NavigationRegion3D.get_rid} to get the {@link RID} associated with a {@link NavigationRegion3D} node. If left empty all regions are included. If a region ends up being both included and excluded at the same time it will be excluded.
   * **Note:** The returned array is copied and any changes to it will not update the original property value. To update the value you need to modify the returned array, and then set it to the property again.
   */

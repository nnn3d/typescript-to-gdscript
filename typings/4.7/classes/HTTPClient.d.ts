...
declare class HTTPClient extends RefCounted {
  ...
  /**
   * Returns all response headers as a {@link Dictionary}. Each entry is composed by the header name, and a {@link String} containing the values separated by `"; "`. The casing is kept the same as the headers were received.
   */
  get_response_headers_as_dictionary(): Dictionary;
  /** Returns a {@link Status} constant. Need to call {@link poll} in order to get status updates. */
  get_status(): int;
  /** If `true`, this {@link HTTPClient} has a response available. */
  has_response(): boolean;
  /** If `true`, this {@link HTTPClient} has a response that is chunked. */
  is_response_chunked(): boolean;
  /**
   * This needs to be called in order to have any request processed. Check results with {@link get_status}.
   */
  poll(): int;
  /**
  ...
  /** Represents the size of the {@link Method} enum. */
   ...
   * - {@link METHOD_GET}: The resource has been fetched and is transmitted in the message body.
   * - {@link METHOD_HEAD}: The entity headers are in the message body.
   * - {@link METHOD_POST}: The resource describing the result of the action is transmitted in the message body.
   * - {@link METHOD_TRACE}: The message body contains the request message as received by the server.
   */

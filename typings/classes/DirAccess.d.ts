...
declare class DirAccess extends RefCounted {
   ...
   */
  change_dir(to_dir: string | NodePath): int;
  /**
   ...
   * Returns one of the {@link Error} code constants ({@link OK} on success).
   */
  copy(from_: string | NodePath, to: string | NodePath, chmod_flags?: int): int;
  /** Static version of {@link copy}. Supports only absolute paths. */
  static copy_absolute(from_: string | NodePath, to: string | NodePath, chmod_flags?: int): int;
  /**
   ...
   */
  create_link(source: string | NodePath, target: string | NodePath): int;
  /**
   ...
   */
  static create_temp(prefix?: string | NodePath, keep?: boolean): DirAccess | null;
  /**
   ...
   */
  dir_exists(path: string | NodePath): boolean;
  /**
   * Static version of {@link dir_exists}. Supports only absolute paths.
   * **Note:** The returned [bool] in the editor and after exporting when used on a path in the `res://` directory may be different. Some files are converted to engine-specific formats when exported, potentially changing the directory structure.
   */
  static dir_exists_absolute(path: string | NodePath): boolean;
  /**
   ...
   */
  file_exists(path: string | NodePath): boolean;
  /**
   ...
   * **Note:** The returned directories in the editor and after exporting in the `res://` directory may differ as some files are converted to engine-specific formats when exported.
   */
  static get_directories_at(path: string | NodePath): PackedStringArray;
  /**
   ...
   */
  static get_files_at(path: string | NodePath): PackedStringArray;
  /**
   ...
   */
  is_bundle(path: string | NodePath): boolean;
  /**
   ...
   */
  is_case_sensitive(path: string | NodePath): boolean;
  /**
   ...
   */
  is_equivalent(path_a: string | NodePath, path_b: string | NodePath): boolean;
  /**
   * Returns `true` if the file or directory is a symbolic link, directory junction, or other reparse point.
   * **Note:** This method is implemented on macOS, Linux, and Windows.
   */
  is_link(path: string | NodePath): boolean;
  /**
   ...
   * Returns one of the {@link Error} code constants ({@link OK} on success).
   */
  make_dir(path: string | NodePath): int;
  /** Static version of {@link make_dir}. Supports only absolute paths. */
  static make_dir_absolute(path: string | NodePath): int;
  /**
   ...
   * Returns one of the {@link Error} code constants ({@link OK} on success).
   */
  make_dir_recursive(path: string | NodePath): int;
  /** Static version of {@link make_dir_recursive}. Supports only absolute paths. */
  static make_dir_recursive_absolute(path: string | NodePath): int;
  /**
   ...
   * Returns `null` if opening the directory failed. You can use {@link get_open_error} to check the error that occurred.
   */
  static open(path: string | NodePath): DirAccess | null;
  /**
   * Returns target of the symbolic link.
   * **Note:** This method is implemented on macOS, Linux, and Windows.
   */
  read_link(path: string | NodePath): string;
  /**
   ...
   * Returns one of the {@link Error} code constants ({@link OK} on success).
   */
  remove(path: string | NodePath): int;
  /** Static version of {@link remove}. Supports only absolute paths. */
  static remove_absolute(path: string | NodePath): int;
  /**
   ...
   * Returns one of the {@link Error} code constants ({@link OK} on success).
   */
  rename(from_: string | NodePath, to: string | NodePath): int;
  /** Static version of {@link rename}. Supports only absolute paths. */
  static rename_absolute(from_: string | NodePath, to: string | NodePath): int;
}

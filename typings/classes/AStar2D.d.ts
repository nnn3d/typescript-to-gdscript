...
declare class AStar2D extends RefCounted {
   ...
   * Note that this function is hidden in the default {@link AStar2D} class.
   */
  ...
  /**
   * Called when neighboring enters processing and if {@link neighbor_filter_enabled} is `true`. If `true` is returned the point will not be processed.
   * Note that this function is hidden in the default {@link AStar2D} class.
   */
   ...
   */
  add_point(id: int, position: Vector2 | Vector2i, weight_scale?: float): void;
  /**
   ...
   */
  get_closest_point(to_position: Vector2 | Vector2i, include_disabled?: boolean): int;
  /**
   * Returns the closest position to `to_position` that resides inside a segment between two connected points.
   * The result is in the segment that goes from `y = 0` to `y = 5`. It's the closest position in the segment to the given point.
   */
  get_closest_position_in_segment(to_position: Vector2 | Vector2i): Vector2;
  /**
   ...
   */
  get_point_path(from_id: int, to_id: int, allow_partial_path?: boolean): PackedVector2Array;
  ...
  set_point_position(id: int, position: Vector2 | Vector2i): void;
  /**

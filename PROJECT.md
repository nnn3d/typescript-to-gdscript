...
- [x] TS-to-GD converter (21 fixture tests): classes, properties, methods, signals, enums, control flow, expressions, operators (incl. `gd.ops.rem` for `%`), comments, await, lambdas, match, dict, gd.ops/as/signal/enum, source maps, variadic parameters (`...args`), `gd.is()` primitive type checks

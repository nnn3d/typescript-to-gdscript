...
export function generateSceneTypings(options: GenerateSceneTypingsOptions): string {
  ...
  lines.push('// Provides get_node/get_node_or_null overloads for scene node paths.');
  lines.push('');
  ...
  for (const [, data] of classData) {
    ...
    for (const child of data.children) {
      lines.push(`    get_node(path: "${child.path}"): ${child.type};`);
      lines.push(`    get_node_or_null(path: "${child.path}"): ${child.type};`);
    }
